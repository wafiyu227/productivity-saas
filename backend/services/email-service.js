...
const RESEND_FROM_EMAIL = process.env.RESEND_FROM_EMAIL || 'Teama AI <team@mail.teamaai.xyz>';
...
class EmailService {
  ...
  async sendDailyDigest(userId, userEmail, summaries) {
    try {
      this.ensureResendConfigured();
      ...
      if (!settings || !settings.email_notifications || !settings.daily_digest) {
        logger.info('Daily digest skipped due to user settings', { userId, userEmail });
  ...
  async sendContactMessage(name, email, company, message) {
    try {
      this.ensureResendConfigured();

      logger.info('Sending contact form message', { name, email, company });

      const { data, error } = await resend.emails.send({
        from: this.fromEmail,
        to: ['team@mail.teamaai.xyz'],
        reply_to: email,
        ...
        html: `
          <div style="font-family: sans-serif; line-height: 1.5; color: #333;">
            ...
            <div style="background: #f9fafb; padding: 20px; border-radius: 8px; margin: 20px 0; border: 1px solid #e5e7eb;">
              ...
              <div style="background: white; padding: 15px; border-radius: 6px; border: 1px solid #e5e7eb; white-space: pre-wrap;">${message}</div>
            </div>
          </div>
        `
      });
      ...
      if (error) {
        logger.error('Contact message send error:', error);
        throw error;
      }

      logger.info('✅ Contact message sent', { messageId: data?.id });
   ...
   * @param {string} html - HTML content of the reply
   * @param {string} originalMessageId - The Message-ID of the email you are replying to
   * @param {string[]} previousMessageIds - Optional list of all previous Message-IDs in the thread
   */
  async sendReply(to, subject, html, originalMessageId, previousMessageIds = []) {
    try {
      this.ensureResendConfigured();
      ...
      // References should contain all previous message IDs, including the one we are replying to
      const references = [...new Set([...previousMessageIds, originalMessageId])];
      ...
      // Save outbound message to DB for dashboard visibility
      const { db } = await import('./supabase-client.js');
      
      // Try to find the original message to get thread_id and team/user context
      const { data: originalMsg } = await db.supabase
        .from('messages')
        .select('thread_id, team_id, user_id')
        .eq('message_id', originalMessageId)
        .maybeSingle();

      const threadId = originalMsg?.thread_id || originalMessageId;
      ...
      logger.info('✅ Reply email sent and recorded', { to, messageId: data?.id, threadId });
      return data;
    } catch (error) {
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              <p>Questions? Reply to this email or contact team@mail.teamaai.xyz</p>
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .content { background: #f9fafb; padding: 30px; }
            .position-card { background: white; padding: 30px; border-radius: 12px; text-align: center; margin: 20px 0; box-shadow: 0 2px 8px rgba(0,0,0,0.05); }
            .position-label { margin: 0; color: #666; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; font-weight: 600; }
            .position-number { font-size: 64px; font-weight: bold; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin: 10px 0; line-height: 1; }
            .position-sublabel { margin: 0; color: #666; font-size: 14px; }
            .features { background: white; padding: 20px; border-radius: 8px; margin: 20px 0; }
            .feature { display: flex; align-items: start; margin: 12px 0; }
            .feature-icon { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 12px; flex-shrink: 0; font-weight: bold; font-size: 14px; }
            ...
            .social-share { background: #f0f4ff; border-left: 4px solid #667eea; padding: 16px; margin: 20px 0; border-radius: 4px; }
          </style>
        </head>
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="features">
                <div class="feature">
                  ...
                  <div><strong>We'll keep you updated</strong><br><span style="color: #666; font-size: 14px;">Regular progress updates as we build</span></div>
                </div>
                <div class="feature">
                  <span class="feature-icon">2</span>
                  <div><strong>Early access</strong><br><span style="color: #666; font-size: 14px;">You'll be first in line when we launch</span></div>
                </div>
              </div>
              <div style="text-align: center; margin-top: 30px;">
                <a href="${FRONTEND_URL}" class="button">Learn More About Teama AI</a>
              </div>
            </div>
            <div class="footer">
